...
data HappyAbsSyn t4 t5
	= HappyTerminal (MDLToken)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4
	| HappyAbsSyn5 t5

happyExpList :: Happy_Data_Array.Array Int Int
happyExpList = Happy_Data_Array.listArray (0,42) ([1632,257,16384,0,256,24,12288,512,52224,24608,262,5120,8704,0,0,0,0,0,0,0,13056,8,49184,524,0,18432,6528,4,16,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","Dir","Forward","Rotate","digit","int","Check","If","Then","Else","L","R","';'","'('","')'","%eof"]
        bit_start = st * 19
        bit_end = (st + 1) * 19
...
action_4 (8) = happyShift action_15
action_4 (9) = happyShift action_16
...
happyReduction_1 (HappyTerminal (TokenInt _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2
	)
...
happyReduction_2 (HappyTerminal (TokenDigit _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2
	)
...
happyReduction_4 (HappyTerminal (TokenDigit _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Check happy_var_2
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenForward _ -> cont 6;
	TokenRotate _ -> cont 7;
	TokenDigit _ happy_dollar_dollar -> cont 8;
	TokenInt _ happy_dollar_dollar -> cont 9;
	TokenCheck _ -> cont 10;
	TokenIf _ -> cont 11;
	TokenThen _ -> cont 12;
	TokenElse _ -> cont 13;
	TokenLeft  _ -> cont 14;
	TokenRight _ -> cont 15;
	TokenSeq _ -> cont 16;
...
happyError' :: () => ([(MDLToken)], [String]) -> HappyIdentity a
