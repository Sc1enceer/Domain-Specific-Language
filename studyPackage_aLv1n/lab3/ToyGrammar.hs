...
happyExpList = Happy_Data_Array.listArray (0,149) ([26368,70,2,0,0,13304,2,0,0,0,0,13184,35,16384,0,128,6556,63489,1587,512,0,4,49088,17,0,26368,70,36046,0,0,128,28672,1126,0,1,512,0,24576,0,192,0,7676,14337,563,8,8,0,0,16384,16384,96,0,8192,39936,281,4600,32770,33,52448,8,0,16256,51,18023,65024,132,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","Type","Bool","Int","arr","int","true","false","'<'","'+'","var","if","then","else","lam","let","':'","'='","in","'('","')'","%eof"]
        bit_start = st * 25
        bit_end = (st + 1) * 25
